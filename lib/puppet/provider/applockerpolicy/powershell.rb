...
Puppet::Type.type(:applockerpolicy).provide(:powershell) do
  ...
  <RuleCollection Type='#{@resource[:type]}' EnforcementMode='#{@resource[:enforcementmode]}'>
    ...
    Puppet.debug "powershell.rb::create creating temp file => #{self.tempfile}"
    testfile = File.open(self.tempfile, 'w')
    ...
    ps("Set-AppLockerPolicy -Merge -XMLPolicy #{self.tempfile}")
    File.unlink(self.tempfile)
    Puppet.debug "deleted #{self.tempfile}"
  end
  ...
  <RuleCollection Type='#{@resource[:type]}' EnforcementMode='#{@resource[:enforcementmode]}'>
    ...
    Puppet.debug "powershell.rb::set creating temp file => #{self.tempfile}"
    xmlfile = File.open(self.tempfile, 'w')
